...
def parse_pdf(file_path: str) -> str:
    ...
    Falls back to pydfplumber for pages that yield no text (e.g. image heavy pages).
    """
    ...
    with logfire.span("PDF parsing (local)", filename = file_path):
        try:
            ...
            if not full_text.strip():
                logfire.info(f"No text extracted from {file_path}. File may be fully image-based.")
            else:
                logfire.info(f"Extracted {len(full_text)} charactera from {file_path}.")                            
